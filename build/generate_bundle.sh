...
generate_bundle() {
    ...
    ${OPERATOR_SDK} generate bundle --verbose --package service-telemetry-operator --input-dir deploy --channels ${BUNDLE_CHANNELS} --default-channel ${BUNDLE_DEFAULT_CHANNEL} --manifests --metadata --version "${OPERATOR_BUNDLE_VERSION}" --output-dir "${WORKING_DIR}" >> ${LOGFILE} 2>&1
    popd > /dev/null 2>&1
    ...
    mv "${WORKING_DIR}/manifests/infra.watch_servicetelemetrys.yaml" "${WORKING_DIR}/manifests/infra.watch_servicetelemetrys_crd.yaml"
